...
class QueryCreator {
     static JSONObject createQuery(JSONObject jsonObject){
        JSONObject returnJSON=new JSONObject();
        String query="";
        try {
            switch (jsonObject.getString(Constant.TYPE)) {
                case Constant.TYPE_INSERT:
                    query=makeStringQuery(Constant.TYPE_INSERT,
                            jsonObject.getString(Constant.TABLE_NAME),
                            jsonObject.getJSONArray(Constant.WHERE),
                            jsonObject.getJSONArray(Constant.VALUE));
                    break;
    ...
    private static String makeStringQuery(String type,String table,JSONArray where,JSONArray value){
        ...
        try {
            switch (type) {
                case Constant.TYPE_INSERT:
                    if (table.equals(Constant.STUDENT_METADATA)) {
                        query = type + " INTO " + table + " (" +
                                ...
                                where.get(wherePos++).toString() + ", " +// bssid
                    ...
                    } else if (table.equals(Constant.TEACHER_METADATA)) {
                    ...
                    } else if (table.equals(Constant.LOGIN_METADATA)) {
                        query = "INSERT INTO " + table + " (" +
                                where.get(wherePos++).toString() + ", " +//email
                                where.get(wherePos++).toString() + ", " +//password-hash
